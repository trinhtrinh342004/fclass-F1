...
| Schema/RLS | `supabase/migrations/20260610160000_auth_admin_approval_completion.sql` | Migration cũ còn thiếu `approval_logs`, seed lớp mặc định, và policy classes theo assigned membership. | Thêm migration idempotent với helper functions, RLS policies chặt hơn, approval logs, unique membership, seed `TuWi A1`, realtime publication. |
